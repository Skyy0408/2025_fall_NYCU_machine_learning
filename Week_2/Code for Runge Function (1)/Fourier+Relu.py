...
params = {
    ...
    'w2': jax.random.normal(keys[1], (32, 32)),
    'b2': jax.random.normal(keys[2], (32,)),

    'w3': jax.random.normal(keys[3], (32, 32)),                                                                   # Added
    'b3': jax.random.normal(keys[4], (32,)),                                                                      # Added

    'w4': jax.random.normal(keys[5], (32, 1)),                                                                    # Added
...
def deep_trigonometric_model(params, x):
    x = x.reshape(-1, 1)
    features = jnp.concatenate([jnp.sin(x @ params['w1']), jnp.cos(x @ params['w1'])], axis=1)
    hidden1 = jax.nn.relu(features @ params['w2'] + params['b2'])                                                 # Added
    hidden2 = jax.nn.relu(hidden1 @ params['w3'] + params['b3'])                                                  # Added
